...
gulp.task('clean', function () {
  return gulp.src(distDir + '/*').pipe(gulp.dest(distDir)).pipe(vinyPaths(del));
});

// 发布到新的目录
gulp.task('copy', ['clean', 'css'], function () {
  let lib = gulp.src(srcDir + '/lib/**').pipe(gulp.dest(distDir + '/lib/'));
  let img;
  if (minImg) {
    img = gulp.src([srcDir + '/images/**', '!' + srcDir + '/images/{slice,slice/**}']).pipe(cache(imagemin({
    ...
    img = gulp.src([srcDir + '/images/**', '!' + srcDir + '/images/{slice,slice/**}']).pipe(gulp.dest(distDir + '/images/'));
  }
  let js     = gulp.src(srcDir + '/js/**').pipe(gulp.dest(distDir + '/js/'));
  // 页面索引工具引入资源存放
  let tmp    = gulp.src(srcDir + '/tmp/**').pipe(gulp.dest(distDir + '/tmp/'));
  let readme = gulp.src(srcDir + '/readme.md').pipe(gulp.dest(distDir));
...
gulp.task('archive:zip', ['archive:conf'], function () {
  ...
  return gulp.src([proArchiveDir + '/**/*', '!' + proArchiveDir + '/*.zip'], {base: archiveDir}).pipe(chmod(0o755, 0o40755)).pipe(zip(proSn + '-' + proName + '-' + proTitle + '-' + timeStr + '.zip'))
  // 放入对应的项目目录
  // .pipe(gulp.dest(proArchiveDir))
  // 存放在归档根目录下
...
gulp.task('archive:rm', ['archive:zip'], function () {
  return gulp.src([proArchiveDir]).pipe(gulp.dest(archiveDir)).pipe(vinyPaths(del));
});
...
gulp.task('conf', function () {
  ...
  for (let param in gutil.env) {
    ...
    // 值不相同时才可配置
    if (param != '_' && param != 'pro' && param != 'sn' && !['sn', 'name',
                                                             'title'].includes(param) && proConf[param] != gutil.env[param]) {
...
gulp.task('reload:unzip', ['reload:check'], function () {
  if (tmpTimestamp != 0) {
    return gulp.src(archiveDir + '/' + proArchiveDirName + '-' + tmpTimestamp + '.zip').pipe(unzip({keepEmpty: true})).pipe(gulp.dest(archiveDir));
  }
});
...
gulp.task('css', ['clean'], function () {
  return gulp.src(srcDir + '/css/*.css').pipe(plumber({
    errorHandler: errHandle
  })).pipe(postcss(distProcessors)).pipe(gulp.dest(distDir + '/css'));
});
...
gulp.task('concat', function () {
  console.log('------------------------------------------------------------------');
  return gulp.src(cssArr).pipe(plumber({
    errorHandler: errHandle
  })).pipe(postcss(devProcessors_concat)).pipe(concat('comm.css')).pipe(gulp.dest(srcDir + '/css'));
});

// LESS编译任务
gulp.task('less', function () {
  return gulp.src(lessArr).pipe(plumber({
    errorHandler: errHandle
  })).pipe(sourcemaps.init()).pipe(less()).pipe(sourcemaps.write())
  // concat
  .pipe(postcss(devProcessors_concat)).pipe(concat('comm.css')).pipe(gulp.dest(srcDir + '/css'));
});
...
gulp.task('groupConcat', groupFiles(cssSrc, function (name, files) {
  ...
  .pipe(postcss(getDevProcessorConcatConf(name))).pipe(concat(name + '.css')).pipe(gulp.dest(srcDir + '/css'));
}));
...
gulp.task('grace', ['concat'], function () {
  return gulp.src(srcDir + '/css/comm.css').pipe(plumber({
    errorHandler: errHandle
  })).pipe(postcss(devProcessors_grace)).pipe(gulp.dest(srcDir + '/css'));
});
...
gulp.task('groupGrace:topx', beforeGraceWorks, groupFiles(cssConcatSrc, function (name, files) {
  ...
  // CSS合并，并完成公式计算
  .pipe(postcss(getDevProcessorsGraceConf(name))).pipe(gulp.dest(srcDir + '/css'));
}));
...
gulp.task('init', ['initdir'], function () {
  let src  = gulp.src(initSrcDir + '/**/*').pipe(gulp.dest(proSrcDir + '/'));
  let view = gulp.src(initViewDir + '/**/*').pipe(gulp.dest(proViewDir + '/'));
  let www  = gulp.src(initWwwDir + '/**/*').pipe(gulp.dest(proWwwDir + '/'));
...
gulp.task('dist', ['copy'], function () {
  let distRs = exec('node www/development.js ' + proName + '/app/make', function (error, stdout, stderr) {
    if (error) {
      ...
      let timeStr = moment().format('YYYYMMDDHHmmss');
      // make zip file
      gulp.src(distDir + '/**/*.*').pipe(chmod(0o755, 0o40755)).pipe(zip(proSn + '.' + proTitle + '-' + timeStr + '.zip')).pipe(gulp.dest(distDir));
    }
  });
});
...
gulp.task('release', function () {
  ...
  gulp.src("./package.json").pipe(jseditor({
  ...
  gulp.src(releaseSource, {
    ...
    // step 3
    // zip the releases
    gulp.src([releaseDir + '/**', releaseDir + '/**/*.*', releaseDir + '/**/.*', releaseDir + '/.*']).pipe(chmod(0o755, 0o40755)).pipe(zip(releaseZipName)).pipe(gulp.dest(releaseRootDir)).on('end', function () {
      process.exit();
    });
  });
});
...
gulp.task('update', function () {
  ...
  if (verInfo.length > 0) {
    ...
    if (isNeedUpdate) {
      ...
      gulp.src(newVerPath).pipe(decompress()).pipe(gulp.dest(rootDir)).on('end', function () {
        // 执行 npm install
...
gulp.task('update:pro', function () {
  ...
  // 控制器升级
  let src            = gulp.src(initSrcDir + '/controller/**/*').pipe(gulp.dest(proSrcDir + '/controller/'));
  // 页面公共块升级
  let viewPublic     = vinyFs.src(initViewDir + '/public/**/*').pipe(vinyFs.dest(proViewDir + '/public/', {overwrite: false}));
  // start 视图升级
  let viewStart      = gulp.src(initViewDir + '/app_start.html').pipe(gulp.dest(proViewDir + '/'));
  // start 视图依赖库升级
  let wwwTmp         = gulp.src(initWwwDir + '/src/tmp/**/*').pipe(gulp.dest(proWwwDir + '/src/tmp/'));
  // 关于 start 视图 readme 文件升级
  let wwwReadme      = gulp.src(initWwwDir + '/src/readme.md').pipe(gulp.dest(proWwwDir + '/src/'));
  // less mixins 升级
  let wwwMixins      = gulp.src(initWwwDir + '/src/css/less/mixins/**/*').pipe(gulp.dest(proWwwDir + '/src/css/less/mixins/'));
  // mixins 索引升级
  let wwwMixinsIndex = gulp.src(initWwwDir + '/src/css/less/lib-mixins.less').pipe(gulp.dest(proWwwDir + '/src/css/less/'));
  ...
  if (commLibChoicesArr.length > 0) {
    gulp.src('gulpfile.babel.js').pipe(
      prompt.prompt({
        ...
        gulp.src(res.libs, {base: commLibPath}).pipe(gulp.dest(proLibPath)).on('end', function () {
